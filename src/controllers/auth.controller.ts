...
export const createSession = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).send({ status: true, statusCode: 200, message: 'Login Success', data: { accessToken } })
  } catch (error: unknown) {
...
//   try {
//     const { decode }: VerifyJWTResult = verifyJWT(value.refreshToken)

//     const user = decode && (await findUserByEmail((decode as { email: string }).email))
//     if (!user) {
